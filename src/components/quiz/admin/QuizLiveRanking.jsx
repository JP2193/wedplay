...
export default function QuizLiveRanking({ quizEvent, totalQuestions, onNext, onFinish }) {
  const [players, setPlayers] = useState([])
  const [loading, setLoading] = useState(true)
  const [advancing, setAdvancing] = useState(false)

  const isLast = quizEvent.current_question_index >= totalQuestions - 1

  useEffect(() => { fetchPlayers() }, [quizEvent.current_question_index])
  ...
  async function handleNext() {
    setAdvancing(true)
    const nextIndex = quizEvent.current_question_index + 1
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <h3 className="font-semibold text-gray-800">
          Ranking — Pregunta {quizEvent.current_question_index + 1} de {totalQuestions}
        </h3>
      </div>
      ...
      <div className="pt-2">
        {isLast ? (
          ...
          <button onClick={handleNext} disabled={advancing} className="btn-primary w-full py-4 text-base">
            {advancing ? '...' : `▶ Siguiente pregunta (${quizEvent.current_question_index + 2}/${totalQuestions})`}
          </button>
        )}
      </div>
    </div>
