...
import AdivinaCreateModal from './AdivinaCreateModal'
...
export default function AdivinaDashboard({ session }) {
  ...
  async function handleDelete(e, id) {
    e.stopPropagation()
    if (!window.confirm('¿Eliminar este juego? Se borrarán todas sus preguntas y jugadores.')) return
    setDeletingId(id)
    await supabase.from('adivina_events').delete().eq('id', id)
  ...
  async function handleReset(e, ev) {
    ...
    const { error } = await supabase.rpc('reset_adivina_event', { p_adivina_event_id: ev.id })
    if (!error) setEvents(prev => prev.map(e => e.id === ev.id ? { ...e, status: 'lobby' } : e))
    setResettingId(null)
  }
    ...
    <div className="min-h-screen bg-gray-50">
      <header className="bg-white border-b border-gray-100 px-4 py-4">
        <div className="max-w-2xl mx-auto flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <h1 className="font-semibold text-gray-800">Adivina Quién</h1>
          </div>
      ...
      <main className="max-w-2xl mx-auto px-4 py-6 space-y-4">
        <div className="flex items-center justify-between">
          <h2 className="text-lg font-semibold text-gray-800">Mis juegos</h2>
          <button onClick={() => setShowModal(true)} className="btn-primary py-2 px-4 text-sm">+ Nuevo juego</button>
        </div>
        ...
        {!loading && events.length === 0 && (
          <div className="card text-center py-12">
            ...
            <p className="text-gray-500 text-sm">Aún no tenés juegos creados.</p>
          </div>
        )}
        ...
        {events.map(ev => (
          <div
            key={ev.id}
            onClick={() => navigate(`/adivina/admin/events/${ev.id}`)}
            ...
            <div className="flex items-start justify-between gap-3">
              <div className="flex-1 min-w-0">
                ...
                <p className="text-gray-400 text-xs mt-1">
                  ...
                  <span className="ml-3">{ev.person1_name} vs {ev.person2_name}</span>
