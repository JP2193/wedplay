...
export default function AdivinaJoinScreen({ onJoined }) {
  ...
  async function handleCodeSubmit(e) {
    ...
    if (error || !data) {
      ...
      setError('Este juego ya finalizó.')
    } else {
      setEventData(data)
      setStep('name')
    }
  ...
  async function handleNameSubmit(e) {
    ...
    const { data: existing } = await supabase
      ...
      .select('*')
      .eq('adivina_event_id', eventData.id)
    ...
    const { data: newPlayer, error: insertError } = await supabase
      .from('adivina_players')
      .insert({ adivina_event_id: eventData.id, full_name: fullName, session_token: sessionToken })
      .select().single()
    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-50 via-white to-rose-50 flex items-center justify-center p-4">
      <div className="w-full max-w-sm space-y-6">
        <div className="text-center space-y-2">
          ...
          <h1 className="text-3xl font-semibold text-gray-800">Adivina Quién</h1>
        </div>
        ...
        <div className="card shadow-md">
          {step === 'code' ? (
            <form onSubmit={handleCodeSubmit} className="space-y-4">
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1.5">Código del juego</label>
                <input
                  type="text"
            ...
            <form onSubmit={handleNameSubmit} className="space-y-4">
              ...
              <button type="submit" disabled={loading || !firstName.trim() || !lastName.trim()} className="btn-primary w-full">
                {loading ? 'Un momento...' : 'Unirme al juego'}
              </button>
